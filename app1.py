...
def uploadToServer():
    if request.method == "POST":
        ...
        if "UserID" in session:
            ...
            upload_file(
                os.path.join(
                    r"C:\Users\monish\Desktop\hackofest\UploadFiles", filename
                ),
                hash,
            )
...
def download():
    if request.method == "POST":
        ...
        print(f"filename: {filename}, passwd: {passwd}")
...
def upload_file(file, hash):
    if "UserID" in session:
        ...
        print(opp.stdout.decode("utf-8"))
        filecid = opp.stdout.decode().split()
